...
carcam = FirstPersonController()
carcam.speed = 0
carcam.gravity = 0
carcam.mouse_sensitivity = Vec2(40,10)
...
texture = load_texture('cartex.png')    # more variables and loads the models and textures of the car.
...
ground = Entity(model = 'plane', scale = 600, texture = 'track.png')  # makes the car and ground entities
...
def update():
    global velocity, speed, model, texture,turnrate  # main game function(inbuilt to ursina engine)

    car.position += car.forward * velocity/2
    carcam.position = car.position
    carcam.rotation_x = car.rotation_x
    carcam.rotation_y = car.rotation_y

    if velocity <= -3.49129086168:  # sets a speed limit if car is in reverse
        velocity = -3.49129086168

    if car.y < -0.1 or held_keys['r']:     # checks if r is pressed and resets car position if it is
        car.position = (0,0,0)
        velocity = 0
        car.rotation = 0
    ...
    if held_keys['w']:
        ...
        if not held_keys['s']:
            if not car.y >= 1:          #fix collision detection later
                velocity += 0.03
                velocity *= 0.993

        else: velocity *= 0.97 - 0.01

    else:
        if velocity > 0:
            velocity = (velocity * 0.997) -0.001   # check if w is pressed and the brake isn't and adds velocity to the car, else it will make the car gradually slow down
    ...
    if held_keys['s']:
        ...
        if not held_keys['w']:
         ...
         if velocity < 0:
            velocity = (velocity * 0.997)  +0.001

    if held_keys['a']:
        car.rotation_y -= (4 - velocity/2) * num_to_range(velocity, 0, 4, 0, 1)
        velocity *= 0.995
        turnrate = 60
    elif held_keys['d']:
        turnrate = 120
